...
class Button:
    def __init__(self, x, y, width, height, text, color):
        ...
        self.text = text
        self.color = color
        self.font = pygame.font.Font("MaruBuri-Bold.ttf", 20)
...
class SheepSprite:
    def __init__(self):
        ...
        self.damage = 25
        self.attack_speed = 200  # 공격 딜레이 (ms)
        self.upgrade_cost = 100  # 초기 강화 비용
    ...
    def upgrade(self, game):
        if game.score >= self.upgrade_cost:
            game.score -= self.upgrade_cost
            self.damage *= 1.2  # 데미지 20% 증가
            self.attack_speed = max(50, self.attack_speed * 0.9)  # 공격속도 10% 증가 (최소 50ms)
            self.upgrade_cost = int(self.upgrade_cost * 1.5)  # 다음 강화 비용 50% 증가
            return True
        return False
...
class Game:
    ...
    (450, 150)    # 다시 상단 중앙으로
        ...
        self.upgrade_button = Button(10, SCREEN_HEIGHT - 60, 160, 50, "데미지 증가", (50, 50, 200))
        self.speed_button = Button(180, SCREEN_HEIGHT - 60, 160, 50, "공격속도 증가", (200, 50, 50))
    ...
    def draw(self, screen):
        ...
        # 강화 정보 표시
        info_font = pygame.font.Font("MaruBuri-Bold.ttf", 16)
        damage_text = info_font.render(f"데미지: {sheep.damage:.1f}", True, COLOR_WHITE)
        speed_text = info_font.render(f"공격속도: {1000/sheep.attack_speed:.1f}/s", True, COLOR_WHITE)
        cost_text = info_font.render(f"강화비용: {sheep.upgrade_cost}", True, COLOR_WHITE)
        
        screen.blit(damage_text, (10, SCREEN_HEIGHT - 100))
        screen.blit(speed_text, (180, SCREEN_HEIGHT - 100))
        screen.blit(cost_text, (350, SCREEN_HEIGHT - 100))
...
while game_active:
    for event in pygame.event.get():
        ...
        # 타워 설치
                if event.button == 1 and game.selected_tower:
                    pos = pygame.mouse.get_pos()
                    game.towers.append(Tower(pos, game.selected_tower))
                    game.selected_tower = None
            
        # 강화 버튼 클릭 처리
                if game.upgrade_button.is_clicked(mouse_pos):
                    if sheep.upgrade(game):
                        print("강화 성공!")
                elif game.speed_button.is_clicked(mouse_pos):
                    if sheep.upgrade_speed(game):
                        print("공격속도 강화 성공!")
